    ...
    QwtPlot(parent)
{
    ...
    //this->setAxisScale( QwtPlot::yLeft, -20.0, 100.0 );
...
trendPlotfz::trendPlotfz(QWidget *parent) :
    QwtPlot(parent)
{
    //this->setPalette(QPalette(Qt::black));
    this->baseTime = QDateTime::currentDateTime();
    ...
    this->setAxisAutoScale(QwtPlot::yLeft, true);
    this->setAxisAutoScale(QwtPlot::xBottom, true);
    //this->setAxisScale( QwtPlot::yLeft, 0.0, 100.0 );
    this->setAxisTitle(QwtPlot::yLeft, tr("Current Value"));
...
void trendPlotfz::showCurveItem(const QVariant &itemInfo, bool on, int index)
{
    QwtPlotItem *item = this->infoToItem(itemInfo);
...
void trendPlotfz::initCurves(int curveCnt, QStringList curveTitleList, QList<QColor> colorList)
{
    for(int i = 0 ; i < curveCnt; i++)
    {
        QwtPlotCurve *curve = new QwtPlotCurve(curveTitleList.value(i));
...
void trendPlotfz::addData(int curveNum, QDateTime time, double data)
{
    if(curveNum < 0 || curveNum >= this->curveList.length())
        return;
...
void trendPlotfz::setCurveEnable(int curveNum, bool flag)
{
    if(curveNum < 0 || curveNum >= this->curveList.length())
        return;
...
void trendPlotfz::clearAllCurvesfz()
{
    for(int i = 0 ; i < this->curveXYList.length(); i++)
    {
        curveXY curve_xy = this->curveXYList.value(i);
